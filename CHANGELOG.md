...
- Found, not yet fixed: `n8nChatUiTrigger`'s "Widget Secret" (checked against a body-embedded `webhook_secret`) cannot be satisfied by the real widget builder, which only offers header-based No Auth/JWT/Basic Auth. Tracked in README "Open items" — blocks real use of the trigger's auth until reworked.
